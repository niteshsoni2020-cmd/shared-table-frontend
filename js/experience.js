...
async function shareExperience(exp) {
    const shareData = { title: exp.title, text: `Check out this experience: ${exp.title}`, url: window.location.href };
    if (navigator.share) { try { await navigator.share(shareData); } catch (err) {} } 
    else { navigator.clipboard.writeText(window.location.href).then(() => { showModal("Link Copied", "Share this link!", "success"); }); }
}
...
async function loadSimilar(current) {
    try {
        ...
        const scored = all.filter(e => e.id !== current.id).map(e => {
                let score = 0;
        ...
        if (scored.length > 0) {
            ...
            container.innerHTML = `<h2 class="text-2xl font-bold mb-8 text-gray-900">Explore more like this</h2><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-10">${scored.map(e => `<div onclick="window.location.href='experience.html?id=${e.id}'" class="cursor-pointer group flex flex-col gap-2"><div class="aspect-[4/3] rounded-xl bg-gray-200 overflow-hidden relative"><img src="${getImageForExperience(e)}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" loading="lazy"><div class="absolute top-3 right-3 bg-white/90 backdrop-blur rounded-full px-2 py-1 text-xs font-bold shadow-sm">${e.averageRating > 0 ? '★ ' + e.averageRating.toFixed(1) : 'New'}</div></div><div><h3 class="font-bold text-gray-900 group-hover:text-orange-600 truncate transition">${e.title}</h3><p class="text-xs text-gray-500">${e.city} • ${e.tags[0] || 'Experience'}</p><p class="text-sm font-bold text-gray-900 mt-1">$${e.price} <span class="text-xs font-normal text-gray-500">/ person</span></p></div></div>`).join('')}</div>`;
...
function showContactModal(exp) {
    ...
    if (!token) { showModal("Login Required", "Please login.", "error"); return; }
    ...
    newBtn.addEventListener('click', async () => {
        ...
        try { await fetch(`${API_BASE}/api/contact`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "User", email: "user@sharedtable.com", subject: `Message for Host (${exp.hostName})`, message: `Regarding: ${exp.title}\n\n${msg}` }) }); if(res.ok) { showModal("Sent", "The host has been notified.", "success"); modal.classList.add('hidden'); } } catch(e) { showModal("Error", "Could not send.", "error"); }
...
function showReportModal(exp) {
    ...
    newBtn.addEventListener('click', async () => {
        ...
        try { await fetch(`${API_BASE}/api/contact`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "System", email: "admin@sharedtable.com", subject: `REPORT: ${exp.title}`, message: `Reason: ${reason}` }) }); showModal("Report Received", "Thank you.", "info"); modal.classList.add('hidden'); } catch(e) { showModal("Error", "Network error.", "error"); }
        newBtn.textContent = "Report";
    });
}

// --- BOOKING LOGIC ---
// (Existing helper functions loadReviews, checkBookmarkStatus, updateBookmarkIcon, toggleCurrentBookmark, setupBookingForm, updatePricePreview removed for brevity but assumed present as before)
// Only change below is adding guestNotes to payload
...
document.getElementById("booking-form").addEventListener("submit", async (e) => {
  ...
  try {
     const body = {
         ...
         guestNotes: document.getElementById("guest-notes").value // NEW: Capture Notes
     };
...
// Re-adding removed helpers for copy-paste safety:
